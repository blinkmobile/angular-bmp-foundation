...
suite('$scope.confirmWithReveal() + confirm', function () {
  ...
  test('confirm closes modal', function (done) {
    ...
    modal$.find('button[x-ng-click]').trigger('click');
  });

  test('confirm triggers correct output', function () {
    assert.equal(output$.text(), 'confirmed!');
  });

});

suite('$scope.confirmWithReveal() + cancel', function () {
  var confirm$, modal$, output$;
  ...
  test('cancel closes modal', function (done) {
    $(document).one('closed', '.reveal-modal[data-reveal]', function () {
      assert.equal(modal$.css('display'), 'none');
      setTimeout(done, 300);
    });
    modal$.find('button.secondary').trigger('click');
  });

  test('cancel triggers correct output', function () {
    assert.equal(output$.text(), 'cancelled!');
  });
